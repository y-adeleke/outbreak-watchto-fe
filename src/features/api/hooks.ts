...
import type { CaseStatDto, FacilityListDto, OutbreakDetailDto, OutbreakListDto, OverviewMetrics } from "@/lib/types";
...
export const useOverviewMetrics = () => {
  ...
  const metrics = useMemo<OverviewMetrics | undefined>(() => {
    if (!outbreaksQuery.data || !facilitiesQuery.data || !statsQuery.data) {
      return undefined;
    }

    const totalOutbreaks = outbreaksQuery.data.length;
    const activeOutbreaks = outbreaksQuery.data.filter((o) => o.isActive).length;
    const resolvedOutbreaks = totalOutbreaks - activeOutbreaks;
    const impactedFacilities = new Set(outbreaksQuery.data.filter((o) => o.isActive).map((o) => o.facilityName)).size;
    const totalResidentCases = statsQuery.data.reduce((sum, stat) => sum + stat.residentCases, 0);
    const totalStaffCases = statsQuery.data.reduce((sum, stat) => sum + stat.staffCases, 0);
    const fatalities = statsQuery.data.reduce((sum, stat) => sum + stat.deaths, 0);
