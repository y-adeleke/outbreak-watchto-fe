...
export function DataPreview() {
  ...
  return (
    <Section eyebrow="Live preview" title="See the API output without leaving the page" description="The widgets below run against the same REST endpoints your applications can consume.">
      <div className="space-y-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm uppercase tracking-wide text-primary">/api/outbreaks</p>
            <p className="text-sm text-muted-foreground">Sorted by most recently reported outbreaks.</p>
          </div>
              ...
              {!isLoading &&
                preview.map((outbreak) => (
                  <TableRow key={outbreak.outbreakId}>
                    <TableCell className="font-mono text-sm">#{outbreak.outbreakId}</TableCell>
                    <TableCell>{outbreak.facilityName}</TableCell>
                    <TableCell>{outbreak.outbreakType}</TableCell>
                    <TableCell>
                      <Badge variant={outbreak.isActive ? "default" : "secondary"} className="inline-flex items-center gap-1">
